...
let rec program() = 
  p{
    let! incs = merge_included_files    // Included files to be parsed
    let inc_rules = fst incs            // Merge rules for files
    let inc_ctxt = snd incs             // TODO: Uitzoeken
    let! imps = import_stmts            // Import statements (like included source files etc.)
    let! initialContext = getContext()  // TODO: Uitzoeken
    let keywordParsingKS = 
      [
        // Defining the tokens for this meta programming language.
    ...
    // Creates a context. Sort the keywords by name.
    do! setContext 
          { initialContext
              with
    ...
    // Set the context with all the found keywords, relationships and imported modules.
    let ctxt = 
          {
...
// If a line contains 2 '/' ignore the rest of the line until you find the char '\n'
...
// Search for the word "import" followed by a blank space. Import the next word.
and import_stmt() =
    p {
        let! bs = blank_space()
        let! w = word "import"
...
// Search for the word "include" followed by a blank space. Include the next word.
and included_file()=
    p {
        let! bs = blank_space()
        let! w = word "include"
...
// Check if the indentation is of the right depth.
